...
def run(
    ...
    Args:
      ...
      true_color: true color for detected object.
    ...
    # Convert to true color to tuple
    atuple = tuple(true_color.strip('()').split(','))
    true_color = (int(atuple[0]), int(atuple[1]), int(atuple[2]))
    ...
    while cap.isOpened():
        ...
        # If something detected, record the time and detect it
            ...
            if (time.time() - old_cmr_time) >= constant.LIMIT_CMR_TIME:
                # Clear lists
                utils.reset_list(color_list, index_list, score_list)
                # Record camera time
                cmr_time = time.time()
                # update old values
            ...
            if time.time() <= (old_cmr_time + constant.LIMIT_DET_TIME):
                probability, index, color = utils.detect_color(
                    detection_result)
                utils.update_list([color_list, index_list, score_list], [
                    color, index, probability])
        ...
        # Else, if only detected on IR
            ...
            # color
            if is_multiple:
                true_color = input("True color of the object: ")
                atuple = tuple(true_color.strip('()').split(','))
                true_color = (int(atuple[0]), int(atuple[1]), int(atuple[2]))
            # Append every lists
        ...
        if ((score_list) and (time.time() > (
            ...
            # print messages
            utils.print_color(color_list[object_id], score_list[object_id])
            ...
            utils.update_list([final_color_list, final_score_list, true_color_list],
                              [color_list[object_id], score_list[object_id], true_color]
                              )
            # Update flag
            print_message = False

        # Draw keypoints and edges on input image based on object color
        image = utils.visualize(image, detection_result, (width, height), color)
        ...
        if cv2.waitKey(1) == 27:
            break
        cv2.imshow('color_detector', image)
    ...
    return (delay_list, fps_list, detected_list,
            final_score_list, final_color_list, true_color_list)
...
def main():
    ...
    parser.add_argument(
        '--trueColor',
        help='(Blue, Green, Red) of the object to be detected if it is a single type.',
        required=False,
        type=str,
        default='(55, 232, 254)')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=False,
        default='./csv_data/Deteksi_Warna.csv')
    parser.add_argument(
        '--showMean',
    ...
    (dly_list,
     ...
     scr_list,
     clr_list,
     true_clr_list) = run(bool(args.multipleObject),
                          str(args.trueColor),
                          args.model,
                          int(args.cameraId),
                          args.frameWidth,
    ...
    record_arr = utils.stack_array(dly_list[0:len(dly_list)], fps_list[0:len(dly_list)],
                                   ...
                                   clr_list[0:len(dly_list)], true_clr_list[0:len(dly_list)]
                                   )
    utils.create_csv(record_arr,
                     ["Delay",
                      "FPS",
                      "Detected",
                      "Probability",
